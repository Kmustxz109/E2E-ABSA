...
class BertABSATagger(BertPreTrainedModel):
    def __init__(self, bert_config):
        ...
        self.model = self._make_model(4)
        # self.ggg = GroupAttention(d_model)##################################
    ...
    def _make_model(self, N=4,
        ...
        group_attn = GroupAttention(d_model)  ##################### need
        # print('group_attn:',group_attn)
        ff = PositionwiseFeedForward(d_model, d_ff, dropout)
        # position = PositionalEncoding(d_model, dropout)
        # word_embed = nn.Sequential(Embeddings(d_model, vocab_size), c(position))
        model = Encoder(EncoderLayer(d_model, c(attn), c(ff), group_attn, dropout),  ############### need
                        N, d_model)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        tagger_input, break_probs = self.model.forward(tagger_input)
        # tagger_input, _ = self.ggg(tagger_input, attention_mask)####################################
