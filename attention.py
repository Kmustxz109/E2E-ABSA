...
def attention(query, key, value, mask=None, dropout=None, group_prob=None, no_cuda=True):
    ...
    # print('p_attn:', p_attn)
    # print('p_attn_size:', p_attn.size())
    return torch.matmul(p_attn, value), p_attn


class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, no_cuda=True):
        ...
        self.d_k = d_model // h
        self.h = h
        self.linears = clones(nn.Linear(d_model, d_model), 4)  ################复制4遍,4个全连接层
        self.attn = None
...
class GroupAttention(nn.Module):
    def __init__(self, d_model, dropout=0.8, no_cuda=True):
    ...
    def forward(self, context,prior):
        batch_size, seq_len = context.size()[:2]

        context = self.norm(context)
        # print("contex:", context.shape)
        ...
        neibor_attn = prior + (1. - prior) * neibor_attn  ########################### Hierarchical Constraint层次约束
        ...
        # print('g_attn:', g_attn)
        # print('g_attn_size:', g_attn.size())
        # save_file1 = open('outputs5_file.txt', mode='w+')
        # att_list = g_attn.numpy().tolist()
        # for i in att_list:
        #     save_file1.write('[')
