...
def evaluate(args, model, tokenizer, mode, prefix="", save_file=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        iter_data = iter(eval_dataloader)
        ...
        for l in batch:
            ...
            # print("line:", l_list)
            for i in l_list:
                save_file.write(','.join([str(a) for a in i]))
                save_file.write('\n')
        save_file.write('\n')
        ...
        with torch.no_grad():
            ...
            # print("outputs_type:", type(outputs))
            o_i = outputs[1]
            # print('o_i:', o_i)
            o_list = o_i.numpy().tolist()
...
def main():
    ...
    #Save vec
    save_file = open('outputs4_file.txt', mode='w+')
    # Validation
    results = {}
    best_f1 = -999999.0
    ...
    for checkpoint in checkpoints:
        ...
        dev_result = evaluate(args, model, tokenizer, mode='dev', prefix=global_step, save_file=save_file)
